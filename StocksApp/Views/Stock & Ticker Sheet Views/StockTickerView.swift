...
struct StockTickerView: View {
    ...
    @ViewBuilder
    private var quoteDetailRow: some View {
...
struct StockTickerView_Previews: PreviewProvider {
    ...
    static var tradingStubsQuoteVM: TickerQuoteViewModel = {
        ...
        mock.stubbedFetchQuotesCallback = { [Quote.stub(isTrading: true)] }
    ...
    static var closedStubsQuoteVM: TickerQuoteViewModel = {
        var mock = MockStocksAPI()
        mock.stubbedFetchQuotesCallback = { [Quote.stub(isTrading: false)] }
        return TickerQuoteViewModel(ticker: .stub, stocksApi: mock)
    }()
    
    static var loadingStubsQuoteVM: TickerQuoteViewModel = {
        var mock = MockStocksAPI()
        mock.stubbedFetchQuotesCallback = { await withCheckedContinuation { _ in } }
        return TickerQuoteViewModel(ticker: .stub, stocksApi: mock)
    }()
    ...
    static var chartVM: ChartViewModel {
            ChartViewModel(ticker: .stub, stocksApi: MockStocksAPI())
        }
